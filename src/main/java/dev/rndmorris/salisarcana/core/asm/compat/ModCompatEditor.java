...
public class ModCompatEditor implements IAsmEditor {
     ...
     * {@code getMixins(List<String> loadedCoreMods)} method in the LateMixin class.
     * <p>
     * You can use this for other methods as well, but you will have to extend this class and override
     * {@code edit(MethodNode method)} to target the correct method
     */
    ...
    /**
     * Injects the compatibility check into the method. This is a default editor that modifies the {@code return mixins}
     * statement to {@code ModCompatEditor.getTransformer(className, methodName, methodDesc).apply_compat(mixins);}
     * <p>
    ...
    public void edit(MethodNode method) {
        ...
        for (int i = 0; i < method.instructions.size(); i++) {
            if (method.instructions.get(i).getOpcode() == Opcodes.ARETURN) {
    ...
    public List<String> apply_compat(List<String> mixins) {
        for (String mixin : new ArrayList<>(mixins)) {
            if (compatMap.containsKey(mixin)) {
                if (compatMap.get(mixin).isEnabled()) {
                    mixins.remove(mixin);
                }
            }
        }
    ...
    public static ModCompatEditor getTransformer(String className, String methodName, String methodDesc) {
        for (ModCompatEditor editor : transformers) {
            if (editor.getClassName().equals(className) && editor.getMethodName().equals(methodName)
                    && editor.getMethodDesc().equals(methodDesc)) {
                return editor;
            }
        }
        return null;
